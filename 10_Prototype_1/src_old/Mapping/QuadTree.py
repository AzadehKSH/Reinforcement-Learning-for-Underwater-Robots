...
class Quadtree:
    ...
    def build_kd_tree(self):
        self._kd_tree = KDTree([(cell.x, cell.y) for cell in self.get_all_cells()])

    def find_nearest_neighbour(self, start_point : Cell):
        _, ind =  self._kd_tree.query([(start_point.x, start_point.y)], k=1)
        point = self._kd_tree.get_arrays()[0][ind]

        return Cell(point[0][0][0], point[0][0][1], -1 )
